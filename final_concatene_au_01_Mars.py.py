...
sample_index=np.random.randint(0, len(var_entree_stand), 1000) #Création d'un jeu de données de taille 1000 et de taille X_scaled
...
plt.bar(range(4), diag_var, width=0.8, color='blue') #Tracé du graphique à barre
plt.show()
...
idx_train=dask.array.random.random(len(df_ech.index)) < 0.6
Xtrain, Xvalidtest= X[idx_train], X[~idx_train]
ytrain, yvalidtest= Y[idx_train], Y[~idx_train]
